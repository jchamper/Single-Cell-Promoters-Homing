...
#FigS14 Generate candidate expression plots within a single cell type.####
library(ggplot2)
library(ggpubr)
library(stringr)
...
for (query_cell_type in sig_cell_types) {
  ...
  p <- ggplot(dt, aes(x = group, y = log2(value + 1), color = group)) +
    ...
    scale_x_discrete(labels = c("good" = "Low expression", "bad" = "High expression")) +
...
#Fig4B Generate volcano plots for five cell types####
library(data.table)
library(pbapply)
library(ggplot2)
library(ggrepel)
...
ggplot(result, aes(x = log2FC, y = neg_log10_p, color = color_group)) +
  ...
  labs(title = "Low - High Somatic Expression Groups",
