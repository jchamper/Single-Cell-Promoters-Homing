...
#FigS10 Generate candidate expression plots within a single cell type.####
library(ggplot2)
library(ggpubr)
library(stringr)
...
#Fig3G Generate volcano plots for five cell types####
library(data.table)
library(pbapply)
library(ggplot2)
library(ggrepel)
...
ggplot(result, aes(x = log2FC, y = neg_log10_p, color = color_group)) +
  ...
  labs(title = "High - Low Male Drive Conversion Groups",
